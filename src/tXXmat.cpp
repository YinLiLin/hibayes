...
template <typename T>
SEXP tXXmat_Geno(XPtr<BigMatrix> pMat, const int chisq = 0, const int threads=0, const bool verbose=true){
	...
	int i, j, k;
	bool sparse = false;
	double m1, m2, sum1, sum2, p1, p2, p12, r;
...
// [[Rcpp::export]]
SEXP tXXmat_Geno(SEXP pBigMat, const int chisq = 0, const int threads=0, const bool verbose=true){
...
template <typename T>
SEXP tXXmat_Geno_gwas(XPtr<BigMatrix> pMat, SEXP gwasgeno, const LogicalVector refindx, const NumericVector gwasindx, const int chisq = 0, const int threads=0, const bool verbose=true){
		...
		for (j = 0; j < mgwas; j++){
			if ( ! Progress::check_abort() ) {
				...
				for(i = j + 1; i < mgwas; i++){
					...
					p12 -= sum1 * m2 + sum2 * m1 - indgwas * m1 * m2;
					r = p12 / (p1 * p2);
					ldmat(gwasindx[i], gwasindx[j]) = ldmat(gwasindx[j], gwasindx[i]) = r;
				}
			}
		}
		return wrap(ldmat);
...
// [[Rcpp::export]]
SEXP tXXmat_Geno_gwas(SEXP pBigMat, SEXP gwasgeno, const LogicalVector refindx, const NumericVector gwasindx, const int chisq = 0, const int threads=0, const bool verbose=true){
...
template <typename T>
SEXP tXXmat_Chr(XPtr<BigMatrix> pMat, const NumericVector chr, const int chisq = 0, const int threads=0, const bool verbose=true){
			...
			for (j = 0; j < chrindx.n_elem; j++){
				if ( ! Progress::check_abort() ) {
					...
					for(i = j + 1; i < chrindx.n_elem; i++){
						...
						p12 -= sum1 * m2 + sum2 * m1 - ind * m1 * m2;
						r = p12 / (p1 * p2);
						ldmat(chrindx[i], chrindx[j]) = ldmat(chrindx[j], chrindx[i]) = r;
					}
				}
			}
		}
...
// [[Rcpp::export]]
SEXP tXXmat_Chr(SEXP pBigMat, const NumericVector chr, const int chisq = 0, const int threads=0, const bool verbose=true){
...
template <typename T>
SEXP tXXmat_Chr_gwas(XPtr<BigMatrix> pMat, const NumericVector chr, SEXP gwasgeno, const NumericVector gwaschr, const LogicalVector refindx, const NumericVector gwasindx, const int chisq = 0, const int threads=0, const bool verbose=true){
				...
				for (j = 0; j < chrindx.n_elem; j++){
					if ( ! Progress::check_abort() ) {
						...
						for(i = j + 1; i < chrindx.n_elem; i++){
							...
							p12 -= sum1 * m2 + sum2 * m1 - ind * m1 * m2;
							r = p12 / (p1 * p2);
							ldmat(gwasindx[chrindx[i]], gwasindx[chrindx[j]]) = ldmat(gwasindx[chrindx[j]], gwasindx[chrindx[i]]) = r;
						}
					}
				}
			}
...
// [[Rcpp::export]]
SEXP tXXmat_Chr_gwas(SEXP pBigMat, const NumericVector chr, SEXP gwasgeno, const NumericVector gwaschr, const LogicalVector refindx, const NumericVector gwasindx, const int chisq = 0, const int threads=0, const bool verbose=true){
