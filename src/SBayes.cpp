...
Rcpp::List SBayesRR(
	...
	for(int iter = 0; iter < niter; iter++){
		...
		for(int i = 0; i < m; i++){
			...
			if(gi != g[i]){
				if(sparse){
	    			...
	    			for (; start != end; ++start){
	      				r_hat[start.row()] -= (gi - g[i]) * (*start);
	   				}
   				}else{
   					r_hat -= (gi - g[i]) * tXX_ma.col(i);
   				}
			}
        ...
        // }
        varg = (sum(vargx) + s2vara_ * dfvara_) / (dfvara_ + m);
		...
		if(iter >= nburn){
            ...
            if(WPPA){
                for(R_xlen_t w = 0; w < nw; w++){
                    ...
                    if(sparse){
                    	varw = spsubmatmtp(tXX_sp, g, windxi);
                    }else{
                    	varw = arma::as_scalar(g(windxi).t() * tXX_ma(windxi, windxi) * g(windxi));
                    }
                    varw /= (n - 1);
                    wgvei[w] += (varw / vara_);
                    if((varw / vara_) >= wppa){
...
Rcpp::List SBayesB(
	...
	Rcpp::List res = SBayesBpi(sumstat, ldm, pi, niter, nburn, windindx, wppa, vara, dfvara, s2vara, vare, dfvare, s2vare, outfreq, true, verbose);
	return res;
}
...
Rcpp::List SBayesCpi(
	...
	for(int iter = 0; iter < niter; iter++){
        ...
        varg = (sum(vargx) + s2vara_ * dfvara_) / (dfvara_ + NnzSnp);
...
Rcpp::List SBayesC(
	...
	Rcpp::List res = SBayesCpi(sumstat, ldm, pi, niter, nburn, windindx, wppa, vara, dfvara, s2vara, vare, dfvare, s2vare, outfreq, true, verbose);
	return res;
}

// [[Rcpp::export]]
Rcpp::List SBayesLASSO(
	const NumericMatrix sumstat, 
	const SEXP ldm,
	const int niter = 50000,
	const int nburn = 20000,
	...
	for(int iter = 0; iter < niter; iter++){
		...
		for(int i = 0; i < m; i++){
			...
			if(gi != g[i]){
				if(sparse){
	    			...
	    			// double sumvare = 0;
	    			// for (InIterMat it(tXX_sp, i); it; ++it){
        ...
        // 			}
	    			for (; start != end; ++start){
   					...
   					r_hat = r_hat - (gi - g[i]) * tXX_ma.col(i);
   				}
   				// update snp effect
				g[i] = gi;
			}
